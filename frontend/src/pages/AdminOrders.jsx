...
export default function AdminOrders() {
  ...
  const load = useCallback(() => {
    const query = { page, ...(tab !== 'all' && { status: tab }), ...(search && { q: search }), ...(userFilter && { user: userFilter.id }) }
  ...
  const act = async (kind) => {
    ...
    setBusy(true)
    try {
      const body = kind === 'complete' ? { txHash: txHash.trim() } : { reason: reason.trim() }
  ...
  return (
    <div style={{ paddingTop: 68 }}>
      <div style={{ background: 'var(--bg-secondary)', borderBottom: '1px solid var(--border)', padding: '28px 0' }}>
        <div className="container" style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', gap: 16, flexWrap: 'wrap' }}>
          <div>
            ...
            <p style={{ color: 'var(--text-secondary)', fontSize: 14 }}>Confirm naira payments and record crypto payouts</p>
          </div>
          <form onSubmit={(e) => { e.preventDefault(); setPage(1); setSearch(query.trim()) }} style={{ display: 'flex', gap: 8 }}>
            <div style={{ position: 'relative' }}>
              <Search size={15} style={{ position: 'absolute', left: 12, top: '50%', transform: 'translateY(-50%)', color: 'var(--text-muted)' }} />
              <input className="input-field" placeholder="Reference, email, address, tx hash" value={query}
          ...
          result.data.map((o) => (
            <div key={o.id} className="card" style={{ marginBottom: 12, padding: 20 }}>
                ...
                <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
                  ...
                  <div>
                    <div style={{ fontWeight: 800, fontSize: 16 }}>
                      {formatCrypto(o.cryptoAmount, o.symbol)} <span style={{ color: 'var(--text-muted)', fontWeight: 500 }}>for</span> {formatNaira(o.amountNgn)}
                    </div>
              ...
              <div style={{ display: 'flex', gap: 8, marginTop: 16, flexWrap: 'wrap' }}>
                {o.receipt?.id
                  ? <button className="btn btn-secondary btn-sm" onClick={() => setReceiptFor(o)}><FileText size={14} /> View receipt</button>
                  : <span style={{ fontSize: 13, color: 'var(--text-muted)', alignSelf: 'center' }}>No receipt yet</span>}
                {ACTIONABLE.includes(o.status) && (
                  <>
                    <button className="btn btn-green btn-sm" onClick={() => { setCompleteFor(o); setTxHash('') }}><CheckCircle2 size={14} /> Mark as sent</button>
      ...
      <Modal open={!!completeFor} onClose={() => setCompleteFor(null)} title="Mark order as sent">
        {completeFor && (
